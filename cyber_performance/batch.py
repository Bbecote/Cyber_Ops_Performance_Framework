#!/usr/bin/env python3
"""
Batch file for the Mesa based ABM
...
def get_availability_index(self):
    ...
    return round((total_node_count - self.exploited_count - self.offline_count)  / total_node_count, 4)
...
params = { 
    "node_count": list(range(1, 31, 5)), #list(range(1,102,5)),
    #"node_count": 15,

    #"security_strength": list(range(1,102,5)),
    "security_strength": 50,

    #----------Friendly
    #"friendly_efficacy": list(range(1,11,1)),
    "friendly_efficacy": 7,
        
    "friendly_count": list(range(1,16,1)),
    #"friendly_count": 2,

    #"friendly_skills": list(range(1,11,1)),
    "friendly_skills": 7,

        #----------Adversary
    "adversary_count": list(range(1,16,1)),
    #"adversary_count": 4, 

    #"adversary_skills": list(range(1,11,1)),
    "adversary_skills": 7,
    }
 

if __name__ == "__main__":
    data = mesa.batch_run(Cyber, params, iterations=20, max_steps=1000, data_collection_period=-1)
    batch = pd.DataFrame(data)
    batch.to_csv("GSA_count__Manova.csv")
    #print(batch.Availability_Index)
    #plt.scatter(batch.Step, batch.Step)
    #plt.show()
